...
class InceptionI3d(nn.Module):
    ...
    def forward(self, x):
        ...
        fea = x.reshape(x.shape[0], x.shape[1])
...
class VisualI3DModel(nn.Module):
    def __init__(self, modality='A;V;M', video_pretrained = True, task='EX'):
        super(VisualI3DModel, self).__init__()

        self.video_model = InceptionI3d()
        if video_pretrained:
            load_pretrain(self.video_model,r'K:\ABAW2022\models\pretrain\i3d_fold5_epo279_UAR49.44_WAR60.36.pth')
        assert 'V' in modality and 'M' not in modality
        self.task = task
        self.modes = ["clip"]
        self.fc = nn.Sequential(
        nn.Linear(in_features=384+384+128+128,out_features=256),
        nn.BatchNorm1d(256),
    ...
    def forward(self, x):
        clip = x['clip']
        video_model_features = self.video_model.extract_features(clip)
        bs,dim,_,_,_ = video_model_features.shape 
        video_model_features = video_model_features.view(bs,dim)
